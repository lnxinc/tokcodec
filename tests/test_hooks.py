...
HOOKS = Path(__file__).parent.parent / "tokcodec" / "assets" / "claude_code"
...
def test_bash_hook_wraps_noisy_commands():
    ...
    assert cmd.startswith("set -o pipefail; { npm test; }") and "tokcodec - -k log" in cmd


def test_bash_hook_leaves_quiet_and_piped_commands_alone():
    for c in ["ls -la", "git status", "pytest | tail -5", "npm test > out.txt", "tokcodec x.py"]:
...
def test_bash_hook_respects_disable_and_background():
    ...
    assert run_hook("bash_hook.py", p, env={"TOKCODEC_HOOK_DISABLE": "1"}) is None
